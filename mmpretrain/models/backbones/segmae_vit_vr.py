...
class SegMAEVit_VR(VisionTransformer):
    ...
    def get_batch_mask_ratio(self, entropys):
        '''
        accroding to the ehtropys of batch to get the avg masking_ratio for a batch
        Args:
            entropys:
    ...
    def seg_var_ratio_masking(self, x: torch.Tensor, data_sample: List[DataSample]):
        '''
        #图片级别的masking rate，batch 内填充0
        '''
    ...
    def seg_var_ratio_masking_avg(self, x: torch.Tensor, data_sample: List[DataSample]):
        '''
        图片级别的masking rate，batch内masking ratio取平均
        Args:
            x (torch.Tensor): Image with data augmentation applied, which is
                of shape B x L x C.
            data_sample:
        ...
        Returns:
            Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
                masked image, mask and the ids to restore original image.
                  - batch_x_masked (torch.Tensor): masked image.
                  - batch_x_masked (torch.Tensor): mask used to mask image.
                  - batch_x_masked (torch.Tensor): ids to restore original image.
        '''
        ...
        # get batch masking ratio
        entropys = []
        for i in range(N):
            entropys.append(data_sample[i].entropy)
        mask_ratio = self.get_batch_mask_ratio(entropys)
        ...
        return batch_x_masked, batch_mask, batch_ids_restore
    def seg_random_masking(self, x: torch.Tensor, data_sample: List[DataSample]):
        '''
        根据轮次决定masking ratio。线性动态增加ratio
        Args:
            x:
            data_sample:
