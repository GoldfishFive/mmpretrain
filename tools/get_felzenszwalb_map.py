...
def mulpool_imagenet1k_train_FH_entropy(img_path):
    save_root = '/home/data1/group1/datasets/imagenet1k/imagenet_train_FH_500_500_entropy/'
    c = os.path.basename(os.path.dirname(img_path))  # class name
    i = os.path.basename(img_path)  # image name
    np_save = os.path.join(save_root, c, i.replace('JPEG', 'npy'))
    if os.path.exists(np_save):
        print(img_path + 'exited!continue...\n')
        return 0
    ...
    # save_data = {'seg': seg.astype(np.uint8), 'num_parts': len(np.unique(seg)), 'entropy': en}
    save_data = {'seg': seg.astype(np.int16), 'num_parts': len(np.unique(seg)), 'entropy': en}
    np.save(np_save, save_data)

def deal_imagenet1k_train():
    dataset_root = '/home/data1/group1/datasets/imagenet1k/train/'
    save_root = '/home/data1/group1/datasets/imagenet1k/imagenet_train_FH_500_500_entropy/'
    img_list = []
    for c in os.listdir(dataset_root):
        c_root = os.path.join(dataset_root,c)
    ...
    pool = multiprocessing.Pool(processes=10)
    # 维持执行的进程总数为processes，当一个进程执行完毕后会添加新的进程进去
    pool.map(mulpool_imagenet1k_train_FH_entropy, img_list)
